...
public class MyBirth {
    ...
    // alt + insert -> 생성자, 메서드재정의, getter/setter 메서드생성
    ...
    public void setYear(int year) {
        if(year < 1900 || year > 2024) {
            System.out.println("잘못된 연도입력입니다.");
             return;
        }
        this.year = year;
    }
    ...
    public void setMonth(int month) {
        if(month < 1 || month > 12) {
            System.out.println("잘못된 월수입력입니다.");
            return;
        }
        this.month = month;
    }
    ...
    public void setDay(int day) {
        if(day < 1 || day > 31) {
            System.out.println("잘못된 일수입력입니다.");
            return;
        } else { // day 가 1 ~ 31 까지 들어옴
            if(this.month == 0) {
                System.out.println("월셋팅을 먼저 완료해주세요");
                return;
            }
