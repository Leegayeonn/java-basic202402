...
public class 람다스트림정리 {
    ...
    public static List<String> getHealthyDishName(List<Dish> menu) {
        /*
            요리목록에서 칼로리가 400칼로리보다 작은 요리들만 추출해서
            칼로리가 낮은 순으로 오름차 정렬한 후
            그 요리들의 이름만 리스트로 만들고 싶어요~
         */
    ...
    public static List<String> getHealthyDishName2(List<Dish> menu) {
        return menu.stream().filter(dish -> dish.getCalories() < 400) // 400 칼로리 미만만
                .sorted(Comparator.comparing(Dish::getCalories))// 칼로리기준으로 오름차 정렬
