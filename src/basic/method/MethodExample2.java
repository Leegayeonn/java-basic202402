...
public class MethodExample2 {
    ...
    static int calcNumberTotal(int[] nums) {   //배열에 감싸서 보내라!
        int total = 0;
        for(int n : nums) {
            total += n;
        }
    ...
    // 배열은 같은 타입의 데이터만 배열로 묶을수 있음
    static int calcNumberTotal2(int... nums) {   //배열에 감싸서 보내라!
        int total = 0;
        for(int n : nums) {
            total += n;
        }
    ...
    public static void main(String[] args) {
        ...
        total = calcNumberTotal(new int[] {1,2,3,4,5,6,7});  //배열로 포장해서 보내기
        System.out.println("total: " + total);


        // 가변 파라미터에 값을 보낼 때는 콤마로 쭉 나열하시거나 위에처럼 배열로 포장해서 보내도됨.
        total = calcNumberTotal2(1,2,3,4,5,6,7);  //배열로 포장하지않음!!!!!(int 만)
        System.out.println("total = " + total);
    }
}
