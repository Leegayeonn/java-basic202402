...
//        arr[5] = arr[6];
        ...
        // 추후에 생성될 새 배열에 값을 그대로 대입하기 편하게 하기 위해 하는작업

        for (int i = 3; i < arr.length-1; i++) {  // 지우고자하는 배열인덱스~(인덱스 길이-1)만큼
            arr[i] = arr[i+1];
        }
...
//        System.out.println(Arrays.toString(temp));
        ...
        arr = temp;  //temp 배열의 주소값을 arr 에게 넘긴다
        temp = null;  //temp 주소값을 지워서 메모리에서 temp는 자동 소멸되게합니다.
